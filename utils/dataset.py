...
class DataSet(torch.utils.data.Dataset):
    ...
    def __init__(self, system: System, M: np.ndarray, K: np.ndarray,
        ...
        x = torch.from_numpy(self.train_data[0]).view(
            self.data_length, system.x_size)  # Convert to tensors and reshape
        z = torch.from_numpy(self.train_data[1]).view(
            self.data_length, system.z_size)
        # Check if output is vector or scalar
        if self.train_data[2].ndim > 2:
            y = torch.from_numpy(self.train_data[2]).view(
                # y data
                self.train_data[2].shape[0]*self.train_data[2].shape[1], self.train_data[2].shape[2])
        else:
            y = torch.from_numpy(self.train_data[2]).view(
                self.train_data[2].shape[0]*self.train_data[2].shape[1])
        ...
        if PINN_sample_mode == 'split set':
            ...
            self.data_length_ph = self.train_data_ph[0].shape[0] * \
                self.train_data_ph[0].shape[1]
            self.x_data_ph = torch.from_numpy(self.train_data_ph[0]).view(
                self.data_length_ph, system.x_size)
            self.z_data_ph = torch.from_numpy(self.train_data_ph[1]).view(
                self.data_length_ph, system.z_size)
            self.ic_ph = self.train_data_ph[4]
            # Check if output is vector or scalar
            if self.train_data_ph[2].ndim > 2:
                self.output_data_ph = torch.from_numpy(self.train_data_ph[2]).view(
                    # y data
                    self.train_data_ph[2].shape[0]*self.train_data_ph[2].shape[1], self.train_data_ph[2].shape[2])
            else:
                self.output_data_ph = torch.from_numpy(self.train_data_ph[2]).view(
                    self.train_data_ph[2].shape[0]*self.train_data_ph[2].shape[1])
            # --------------------------------------------------------------
            ...
            raise Exception(
                'Sample mode must be either ''split set'', ''split traj'' or ''no physics''.')
    ...
    def generate_data(self, seed: int) -> tuple:
        ...
        # Sample initial conditions for x
        ic = self.system.sample_ic(self.limits_normal, self.samples, seed=seed)
        x_data_fw, output_fw, t_fw = self.system.generate_data(
            ic, self.a, self.b, self.N)      # Forward simulation

        if self.data_gen_mode == 'backward sim':
            _, output_bw, t_bw = self.system.generate_data(
                # Backward simulation
                ic, self.a, t_back, int(np.abs(t_back/h)))
            output_bw = np.flip(output_bw, axis=1)
        elif self.data_gen_mode == 'negative forward':
            _, output_bw, t_bw = self.system.generate_data(
                # Backward simulation
                ic, t_back, self.a, int(np.abs(t_back/h)))
        else:
            raise Exception(
                'Data generation mode must be either ''backward sim'' or ''negative forward''.')

        ic_z_bw = np.random.rand(self.samples, self.system.z_size)
        z_data_fw1 = data.KKL_observer_data(
            self.M, self.K, output_bw, t_back, self.a, ic_z_bw, int(np.abs(t_back/h)))
        ic_z = z_data_fw1[:, -1, :]     # The last element of each trajectory
        # ic_z = np.zeros([samples, self.system.z_size])
        z_data_fw2 = data.KKL_observer_data(
            self.M, self.K, output_fw, self.a, self.b, ic_z, self.N)
