...
class System_z:
    ...
    def z_dynamics(self):
        if self.is_autonomous:
            if self.y_size > 1:
                def z_dot(y, z): return np.matmul(self.M, z) + \
                    np.matmul(self.K, np.expand_dims(y, 1))
            else:
                def z_dot(y, z): return np.matmul(self.M, z) + self.K*y

        else:
            if self.y_size > 1:
                def z_dot(y, q, z): return np.matmul(self.M, z) + \
                    np.matmul(self.K, np.expand_dims(y, 1)) + q
            else:
                def z_dot(y, q, z): return np.matmul(self.M, z) + self.K*y + q

        return z_dot


class Observer:
    def __init__(self, system, z_system, net, a, b, N, init_z_zero=True, device='cuda'):
        self.system = system
        self.z_system = z_system
        self.f = z_system.z_dynamics()
        self.T = net.encoder
        self.T_inv = net.decoder
        self.a = a
        self.b = b
        self.N = N
    ...
    def simulate_NA(self, ic, u0, g):
        ...
        """
        x, y, t = self.system.generate_data(
            ic, self.a, self.b, self.N)     # Generate y data
