...
def main():
    ...
    a = 0       # Starting time
    b = 50      # End time
    N = 1000    # runge_kutta4 intervals
    num_ic = 50       # Number of initial conditions to be sampled
    # Sample space for normal datapoints
    limits_normal = np.array([[-1, 1], [-1, 1]])

    z_sys5 = System_z(A, B, sys_dim5)
    dataset = DataSet(sys_dim5, A, B, a, b, N, num_ic, limits_normal,
    ...
    observers = []
    for model in models:
        observers.append(observer(sys_dim5, z_sys5, model,
                         a, b, N, init_z_zero=False))
