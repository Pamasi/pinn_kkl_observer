...
class Trainer:
    ...
    lmbda: int
        Constant to weigh the PDE loss

    method: str
        Mehtod of training. Either of supervised_NN, unsupervised_AE or supervised_PINN.
    ...
    def __init__(self, dataset: DataSet, epochs: int, optimizer: torch.optim, net: MainNetwork, loss_fn: torch.nn, batch_size: int, lmbda: int, method: str, shuffle: bool = True, scheduler: Optional[None] = None, reduction: str = 'mean') -> None:
        ...
        self.pde1 = PdeLoss_xz(self.dataset.M, self.dataset.K, self.dataset.system,
                               self.loss_calculator, lmbda, reduction=self.reduction)
            ...
            for idx, data in enumerate(self.trainset):
                ...
                if self.normalizer != None:
                    label_x = self.normalizer.Normalize(
                        x, mode='normal').float()
                    label_z = self.normalizer.Normalize(
                        z, mode='normal').float()
                else:
                    label_x = x
                    label_z = z
